...
import com.cryptodoc.utils.PredicateTreeGenerator;
import com.cryptodoc.utils.TreeNode;
import com.cryptodoc.utils.Utils;
import com.cryptodoc.writer.FreeMarkerWriter;
...
public class DocumentGeneratorMain {
	...
	private static final RulesetReader ruleReader = new RulesetReader();
	...
	public static void main(String[] args) throws IOException, TemplateException, CryptoAnalysisException {
		...
		List<CrySLRule> rules = new ArrayList<>(ruleReader.readRulesFromDirectory(docSettings.getRulesetPathDir()));

		Collections.sort(rules, new Comparator<CrySLRule>() {
			@Override
			public int compare(CrySLRule o1, CrySLRule o2) {
		...
		for (CrySLRule ruleEntry : rules) {
			...
			mapEnsures.put(rule.getClassName(), new ArrayList<>(rule.getPredicates()));
			mapRequires.put(rule.getClassName(), new ArrayList<>(rule.getRequiredPredicates()));
		}
		...
		for (CrySLRule ruleEntry : rules) {
			...
			// ConstraintsSection
			composedRule.setComparisonConstraints(comp.getConstriantsComp(rule));
			...
			//
			List<String> allConstraints = new ArrayList<>(composedRule.getComparisonConstraints());
		...
		// Freemarker Setup and create html pages
		...
		// copy CryslRulesFolder into generated folder
		// specifify this flag to distribute the documentation
		System.out.println("CryptoDOC generated to: " + DocSettings.getInstance().getReportDirectory());
