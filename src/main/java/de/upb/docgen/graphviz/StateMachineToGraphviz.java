...
public class StateMachineToGraphviz {
    ...
    public static String toGraphviz(StateMachineGraph smg) {
        ...
        for (TransitionEdge edge : edges) {
            if (edge.getLeft().getName().equals("-1"))
                stringBuilderToFile.append("Start").append(" -> ").append(edge.getRight().getName());
    ...
    public static void toPNG(String name) {
        try {
            MutableGraph g = new Parser().read(new File("dotFSMs\\" + name + ".dot"));
    ...
    public static void toPNG(String name, String pathToRootpage) {
        try {
            MutableGraph g = new Parser().read(new File(pathToRootpage + "\\" + "dotFSMs\\" + name + ".dot"));
